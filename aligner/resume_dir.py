...
f = open("e_checkpoint 2")
e_count = pickle.load(f)
f.close()
f = open("fe_checkpoint 2")
...
sys.stderr.write('Calculte perplexity')
perplexity = 0
for (n, (f, e)) in enumerate(bitext):
  p_fe = 1
  for f_i in set(f):
    ...
    for e_j in set(e):
      theta[(f_i, e_j)] = theta_fe[(f_i, e_j)] / theta_e[e_j]
      Z += theta[(f_i, e_j)]
    p_fe *= Z

  perplexity += -math.log(p_fe)
...
for (f, e) in bitext:
  for (i, f_i) in enumerate(f):
    ...
    for (j, e_j) in enumerate(e):
      theta = theta_fe[(f_i, e_j)] / theta_e[e_j]
      if theta > max_prob:
        max_prob = theta
        k = j
