...
def get_neighbors(h, tm, obj):
  ...
  for i in xrange(0, len(h)):
    ...
    for j in range(i + 1, len(h)):
      new_hyp = list(h)
      new_hyp[i], new_hyp[j] = new_hyp[j], new_hyp[i]
    ...
    #merge neighbors
    if i < len(h)- 1 and h[i][1] + h[i+1][1] in tm and h[i].end + 1 == h[i+1].start:
...
def score(l, tm, lambda_pe = 1, lambda_pfe = 1, lambda_reordering = 1):
  state = lm.begin()
  e_logprob = 0.
  fe_logprob = 0.
  ...
  return lambda_pe*e_logprob + lambda_pfe*fe_logprob + lambda_reordering*reordering_logprob
...
for f in french:
      ...
      for j in xrange(i+1,len(f)+1):
        if f[i:j] in tm:
          ...
          for phrase in tm[f[i:j]]:
            ...
            logprob = h.logprob + lambda_pfe *phrase.logprob
            lm_state = h.lm_state
            for word in phrase.english.split():
              (lm_state, word_logprob) = lm.score2(lm_state, word)
              logprob += lambda_pe * word_logprob

            # add 'reordering'
            logprob += lambda_reordering*abs(i - lm_state[1] - 1) * math.log(0.9)
          ...
          if f[i:k] in tm and f[k:j] in tm:
            for phrase1 in tm[f[i:k]]:
              for phrase2 in  tm[f[k:j]]:
                ...
                logprob1 = h.logprob + lambda_pfe *phrase2.logprob
                lm_state1 = h.lm_state
                for word in phrase2.english.split():
                  (lm_state1, word_logprob) = lm.score2(lm_state1, word)
                  logprob1 += lambda_pe *word_logprob
                logprob1 += lambda_reordering*abs(k - lm_state[1] - 1) * math.log(0.9)
                lm_state1 = (lm_state1[0], j - 1)

                logprob2 = logprob1 + lambda_pfe *phrase1.logprob
                lm_state2 = lm_state1
                for word in phrase1.english.split():
                  (lm_state2, word_logprob) = lm.score2(lm_state2, word)
                  logprob2 += lambda_pe * word_logprob


                # add reordering term
                # logprob += abs(k - lm_state[1] - 1) * math.log(0.9)
                logprob2 += lambda_reordering*abs(i - (j-1) - 1) * math.log(0.9)
  ...
  # for cur in current:
    # print(extract_english(cur))
  current = [(tuple(cur), score(cur, tm)) for cur in current]
  ...
  for _ in range(20):
    ...
    for cur, _ in current:
      cur = list(cur)
      for h in get_neighbors(cur, tm, obj):
        c = score(h,tm, lambda_pe, lambda_pfe, lambda_reordering)
        s_current.append((tuple(h),c))
    s_current = list(set(s_current))
    s_current.sort(key=lambda x:-x[1])

    current = s_current[:opts.num_greedy]
    # winner = current[0][0]
    # print(winner)
    # print(current)
    # for cur in current:
      # print(extract_english(cur[0]), cur[1])
