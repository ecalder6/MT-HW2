...
parser.add_argument("--load_file", default='',
                    help="Location to dump the source model.")
...
def main(options):
  ...
  if os.path.isfile(options.load_file):
    nmt = torch.load(open(options.load_file, 'rb'))
  else:
    nmt = NMT(src_vocab_size, trg_vocab_size, use_cuda)
    # trg_lm = NMT(trg_vocab_size, trg_vocab.stoi['<s>'], trg_vocab.stoi['</s>'], options.embedding_size, options.hidden_size, options.dropout, use_cuda)
